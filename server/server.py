...
def gen():
	print('test')
	# socketio.start_background_task(gen_frame)

def gen_frame():
	if camera.Video is None:
		return
	while(True):
		ret,img=camera.Video.read()
		if ret:
			ret, buffer = cv2.imencode('.jpg', img)
			socketio.emit('video_feed', base64.encodebytes(buffer.tobytes()).decode("utf-8") )
			socketio.sleep(0)
		else:
			print("Hit here, not connected")
			break
